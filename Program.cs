...
namespace Discord01
{
    class Program
    {
        ...
        public void CreateCommands()
        {
            ...
            cService.CreateCommand("du")
                .Description("Stats from PUBG tracker online for duos")
                .Parameter("user", Discord.Commands.ParameterType.Unparsed)
             .Do(async (e) =>
             {
                 await e.Channel.SendIsTyping();
             ...
             try
             {
                 var duoRank = stats.Stats.Find(x =>
                     x.Mode == Mode.Duo && x.Region == PUBGSharp.Region.NA && x.Season == Season.EASeason2).Stats.Find(x =>
                         x.Stat == Stats.Rating).Rank;
            ...
            cService.CreateCommand("so")
                .Description("Stats from PUBG tracker online for solos")
                .Parameter("user", Discord.Commands.ParameterType.Unparsed)
                .Do(async (e) =>
                {
                    await e.Channel.SendIsTyping();
                    ...
                    try
                    {
                        ...
                        var soloRank = stats.Stats.Find(x =>
                            x.Mode == Mode.Solo && x.Region == PUBGSharp.Region.NA && x.Season == Season.EASeason2).Stats.Find(x =>
                                x.Stat == Stats.Rating).Rank;
